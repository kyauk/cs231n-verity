...
:func:`pipeline.modules.debate.react_loop.run_actor`. Produces a
:class:`DebateOutputRecord` whose rich per-actor data is stashed in
``metadata`` under namespaced keys so downstream consumers (proposal builder,
frontend) can read it directly.

Model calls are made exclusively through the injected
:class:`pipeline.modules.debate.config.TextLLMClient` /
:class:`pipeline.modules.debate.config.VLMClient` protocols (lego rule).
"""
...
from pipeline.modules.debate.config import DebateConfig, TextLLMClient, VLMClient
from pipeline.modules.debate.react_loop import ActorContribution, run_actor
from pipeline.modules.debate.scoring import (
...
from pipeline.modules.debate.tools import DebateContext


# ---------------------------------------------------------------------------
# Debate output record (module-local; consumed by proposals.py + Debater)
# ---------------------------------------------------------------------------


@dataclass
class DebateOutputRecord:
    """One debate output row.

    Mirrors the legacy ``models.handoff_contracts.DebateOutputRecord`` shape so
    the proposal builder and orchestrator can consume it unchanged, but stays
    inside the debate lego module (no cross-module/legacy import).
    """
...
def _fallback_scene_output(record: DebateInput) -> dict[str, Any]:
    return {
...
def _fallback_risk_output(
    ...
    from pipeline.modules.debate.tools import safety_taxonomy_lookup
...
def _make_actor_llm_call(text_client: TextLLMClient, temperature: float):
    """Return an llm_call callable bound to the injected text-LLM client.

    Every actor turn routes through ``text_client.complete`` at the configured
    debate temperature — the sole text-model seam for the actor loop.
    """

    def _call(messages: list[dict[str, Any]]) -> str:
        return text_client.complete(messages, temperature=temperature)
...
def run_tool_augmented_debate(
    ...
    Returns a :class:`DebateOutputRecord` alongside the raw ``proposal_metadata``
    dict so the proposal builder can adopt the richer per-actor fields directly.

    ``text_client`` / ``vlm_client`` / ``config`` are injected dependencies; the
    debate engine holds no model endpoint of its own.
    """
    ...
    max_steps = int(config.max_actor_steps)
    llm_call = _make_actor_llm_call(text_client, config.temperature)
    ...
    proposal_metadata: dict[str, Any] = {
        ...
        "actor_contributions": [c.to_dict() for c in all_contributions],
