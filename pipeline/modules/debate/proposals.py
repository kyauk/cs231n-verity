...
The four-actor debate (:mod:`pipeline.modules.debate.actors`) stashes rich
per-actor structured output under namespaced ``proposal_*`` keys inside the
debate output record's ``metadata``. This module consumes those keys and
assembles a first-class
:class:`pipeline.interfaces.debate.RegressionCaseProposal` that downstream
consumers can render directly without digging through metadata.
"""
...
from pipeline.interfaces.debate import RegressionCaseProposal
from pipeline.modules.debate.actors import DebateOutputRecord
