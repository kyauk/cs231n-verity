...
:func:`pipeline.modules.debate.tools.execute_tool`, and returns a structured
...
:mod:`pipeline.modules.debate.actors` so this loop can stay reusable.
"""
...
from pipeline.modules.debate.tools import (
    DebateContext,
    TOOL_DESCRIPTIONS,
    execute_tool,
)
...
class ToolCall:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"tool_name": self.tool_name, "tool_input": dict(self.tool_input)}
...
class ActorContribution:
    ...
    # Back-compat alias for callers that used the pydantic ``model_dump`` API.
...
def run_actor(
    ...
    ``llm_call`` and the model-touching tools dispatched through ``context``
    are the only model seams; this loop holds no endpoint of its own.
    """
