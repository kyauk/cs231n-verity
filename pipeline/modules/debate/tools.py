...
Model-touching tools (``vlm_followup``) go exclusively through the injected
:class:`pipeline.modules.debate.config.TextLLMClient` /
:class:`pipeline.modules.debate.config.VLMClient` protocols carried on the
:class:`DebateContext` — never a hardcoded endpoint (lego rule).
"""
...
class DebateContext:
    ...
    Attributes:
        media_refs: Absolute/relative paths or URLs to the video and/or image
            files that describe the flagged window.
        ...
        text_client: Injected text-LLM client for text-only follow-up fallback.
        vlm_client: Injected VLM client for live visual follow-ups.
    """
...
def _vlm_followup_via_text(
    ...
    """Answer a visual follow-up using the prior description (no live VLM).

    ``reason`` is interpolated into the observation tag so the transcript shows
    *why* the text-only path was used (``disabled`` when
    ``REACT_ENABLE_VLM_FOLLOWUP`` is off, ``vlm-failed`` when a live VLM attempt
    raised). This lets a reviewer distinguish "feature off" from
    "VLM path crashed and fell back". The text answer is produced through the
    injected text-LLM client.
    """

    if context.text_client is None:
        return "vlm_followup error: no text client available for text-only follow-up."
    ...
    try:
        answer = context.text_client.complete(messages).strip()
    except Exception as error:  # noqa: BLE001
        return f"vlm_followup error: {error}"
    return f"[text-only:{reason}] {answer}"


def _vlm_followup_via_vlm(question: str, context: DebateContext) -> str:
    """Run a live VLM follow-up against the injected VLM client.

    The first media ref that looks like a video/image is used as the clip
    reference. The injected ``vlm_client.followup`` is responsible for fetching
    and encoding the clip; this tool only routes the targeted question to it.
    """

    if context.vlm_client is None:
        return "vlm_followup error: no VLM client available for live follow-up."

    video_ref: str | None = None
    for media_ref in context.media_refs:
        lowered = media_ref.lower()
        if lowered.endswith(
            (".mp4", ".mov", ".mkv", ".avi", ".webm", ".png", ".jpg", ".jpeg", ".webp")
        ):
            video_ref = media_ref
            break
    ...
    return context.vlm_client.followup(video_ref, prompt).strip()


def vlm_followup(tool_input: dict[str, Any], context: DebateContext) -> str:
    """Ask a targeted follow-up about the scene (text-only by default; optional live VLM)."""
    ...
    try:
        return _vlm_followup_via_vlm(question, context)
    except Exception as error:  # noqa: BLE001
        # Any VLM-side failure (endpoint outage, missing weights, video decode
        # error, ...) falls back to text-only so the actor still gets a usable
        # Observation. Surface the underlying error in the tag so the
        # transcript shows what broke.
        error_text = str(error)
        lowered = error_text.lower()
        if "out of memory" in lowered or "cuda" in lowered:
            short_reason = "vlm-oom"
        else:
            short_reason = "vlm-failed"
        print(
            f"[debate] vlm_followup live path failed ({short_reason}): "
            f"{error_text}",
            flush=True,
        )
        ...
        return f"{text_answer} (VLM error: {truncated})"
