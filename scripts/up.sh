...
#
#   NIM (Cosmos-Reason, :8081)  +  judge_ui (:8001)
...
#
# Credentials: :8001 judge reads the OUTPUT bucket and SIGNS video URLs -> uses
# the SA key (a user refresh-token cannot sign v4 URLs).
set -euo pipefail
...
echo "[1/3] NIM container"
docker compose --profile gpu up -d cosmos-reason2 >/dev/null 2>&1 || true

# 2) judge_ui :8001 — SA key (reads output bucket + signs URLs)
echo "[2/3] judge API :8001"
