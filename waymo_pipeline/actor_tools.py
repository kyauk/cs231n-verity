...
def _vlm_followup_enabled() -> bool:
    # API-backed follow-ups are cheap (no local GPU), so they default ON here.
    return os.getenv("REACT_ENABLE_VLM_FOLLOWUP", "1").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _vlm_followup_via_text(
    ...
    *why* the text-only path was used (``disabled`` when
    ``REACT_ENABLE_VLM_FOLLOWUP`` is off, ``no-media`` when no clip is
    available, ``api-failed`` when the hosted VLM call raised). This lets a
    reviewer distinguish "feature off" from "VLM API call failed and fell
    back".
    """

    from waymo_pipeline.waymo_describe_and_debate import _nim_text_chat_completion
...
def _resolve_followup_media(context: DebateContext) -> tuple[str, str] | None:
    """Return ``(abs_path, kind)`` for the first usable clip/frame in media_refs.

    ``kind`` is ``"video"`` or ``"image"``. Returns ``None`` when no supported
...
def _vlm_followup_via_api(question: str, context: DebateContext) -> str:
    """Answer a visual follow-up using the NVIDIA NIM hosted vision model.

    Unlike the original GPU path, this loads no local weights. The first
    available clip/frame in ``context.media_refs`` is sent to the same NIM VLM
    used for scene description (``DESCRIBE_NIM_MODEL_ID``) as a base64 data
    URI, alongside the targeted question. Raises on any transport/HTTP error so
    ...
    media = _resolve_followup_media(context)
    if media is None:
    ...
    if not api_key:
        raise RuntimeError("NVIDIA_API_KEY missing for vlm_followup.")
    ...
    messages: list[dict[str, Any]] = [
        {
            ...
            "content": (
                ...
                "scene. Answer concisely (<=80 words) grounded only in what is "
                "visible in the clip. If the clip does not show the answer, say "
    ...
    if response.status_code != 200:
        raise RuntimeError(
            f"NIM VLM follow-up failed: HTTP {response.status_code}: "
            f"{response.text[:300]}"
        )
...
def vlm_followup(tool_input: dict[str, Any], context: DebateContext) -> str:
    """Ask a targeted follow-up about the scene via the hosted NIM VLM API.

    The clip is re-queried through the same NVIDIA NIM vision model used for
    the initial description -- no local model is ever loaded. On any failure
    (no clip on disk, missing API key, HTTP/transport error) this degrades to a
    text-only answer derived from the prior description so the actor still
    receives a usable Observation.
    """
    ...
    if _resolve_followup_media(context) is None:
        return _vlm_followup_via_text(question, context, reason="no-media")

    try:
        return _vlm_followup_via_api(question, context)
    except Exception as error:  # noqa: BLE001
        # Any API-side failure (missing key, HTTP error, transport timeout,
        # decode error, ...) falls back to text-only so the actor still gets a
        # usable Observation. Surface the underlying error in the tag.
        error_text = str(error)
        print(
            f"[pipeline] vlm_followup API path failed: {error_text}",
            flush=True,
        )
        text_answer = _vlm_followup_via_text(
            question, context, reason="api-failed"
        )
        truncated = error_text if len(error_text) <= 200 else error_text[:197] + "..."
        return f"{text_answer} (VLM API error: {truncated})"
